...
export default function RocketTransition({ isActive, onComplete }: RocketTransitionProps) {
  const [phase, setPhase] = useState<'idle' | 'appear' | 'launch' | 'zoom'>('idle')

  const stableOnComplete = useCallback(onComplete, [onComplete])

  useEffect(() => {
    if (isActive) {
      // Phase 1: Rocket appears at bottom-left (0-1s)
      setPhase('appear')
    ...
    <AnimatePresence>
      {isActive && (
        ...
        >
          ...
          {/* Countdown 3-2-1 during appear phase */}
          <motion.div
          ...
          >
            <motion.span
              className="text-6xl sm:text-8xl font-black text-fire-gradient"
